...
import { initReviews } from './reviews.js'; // <--- IMPORT MODULE REVIEWS

// ============================================================
// 1. STATE CHO TRANG CHI TIẾT SẢN PHẨM
// ============================================================
let productState = {
    variants: [],
    selectedColor: null,
...
function renderSkeleton(count = 6) {
    return Array(count).fill(0).map(() => `
      <div class="bg-white rounded-xl border border-gray-200 p-3 flex flex-col h-full animate-pulse">
        <div class="rounded-lg bg-gray-200 h-48 w-full mb-3"></div>
        ...
        variantsHtml = `
            <div class="mt-2 px-1 card-variants-area" 
                ...
                <div class="flex flex-wrap gap-1 text-[10px] capacity-container">
                    ${renderCardCapacities(p.variants, colorList[0])}
                </div>
            </div>
        `;
    }
...
function renderCardCapacities(variants, selectedColor) {
    ...
    return distinctVariants.map((v, idx) => {
        ...
        return `<button onclick="selectCardCapacity(this, ${v.product_id}, ${v.id}, ${v.price}, '${safeImage}')" 
...
window.selectCardColor = (btn, productId) => {
    ...
    let variants = [];
    try {
        variants = JSON.parse(wrapper.dataset.variants.replace(/&apos;/g, "'"));
    } catch(e){ console.error('Parse variants error', e); return; }
    ...
    tabs.forEach((t) => {
        ...
        newTab.addEventListener('click', () => {
            document.querySelectorAll('.tab-btn').forEach((x) => {
                ...
                x.classList.add('text-gray-500', 'border-transparent');
            });
            ...
            // Fix logic mapping ID cho các tab
            if (newTab.dataset.tab === 'description') document.getElementById('description-content').classList.remove('hidden');
            else if (newTab.dataset.tab === 'specs') document.getElementById('specs-content').classList.remove('hidden');
            else if (newTab.dataset.tab === 'reviews') document.getElementById('reviews-content').classList.remove('hidden');
        });
    });
}
...
function renderSpecs(specsJson, specsTemplateJson, category) {
    ...
    let template = [];
    if (specsTemplateJson) { try { template = typeof specsTemplateJson === 'string' ? JSON.parse(specsTemplateJson) : specsTemplateJson; } catch (e) {} }
    ...
    template.forEach((group) => {
        ...
        if (group.fields) {
            group.fields.forEach((field) => {
                ...
                html += `<li class="grid grid-cols-12 gap-2 px-4 py-3 border-b border-gray-100 last:border-0"><span class="col-span-5 text-gray-500 font-medium">${label}:</span><span class="col-span-7 font-semibold text-gray-900 ${!value?'text-gray-400 italic text-xs font-normal':''}" id="spec-val-${key}" data-spec-key="${key}" data-original="${display}">${display}</span></li>`;
            });
        }
        html += `</ul>`;
    });
...
function renderSelectors() {
    ...
    if (colors.length > 0) {
        ...
        colorDiv.innerHTML = `<label class="block text-sm font-bold text-gray-700 mb-2">Màu sắc: <span class="font-normal text-gray-500" id="selected-color-label">${productState.selectedColor}</span></label>`;
        ...
        colors.forEach(color => {
            ...
            const baseClass = "px-3 py-2 rounded-lg border transition flex items-center gap-2 min-w-[80px] justify-center";
            const activeClass = "border-blue-600 bg-blue-50 text-blue-700 font-bold ring-1 ring-blue-600";
            const inactiveClass = "border-gray-300 bg-white text-gray-700 hover:border-blue-400";
            btn.className = `${baseClass} ${isActive ? activeClass : inactiveClass}`;
            btn.innerHTML = `<span class="w-4 h-4 rounded-full border border-gray-200 shadow-sm" style="background-color: ${hexCode}"></span><span>${color}</span>`;
            btn.onclick = () => {
    ...
    if (productState.selectedColor) {
        ...
        if (capacities.length > 0) {
            ...
            capDiv.innerHTML = `<label class="block text-sm font-bold text-gray-700 mb-2">Cấu hình: <span class="font-normal text-gray-500">${productState.selectedCapacity}</span></label>`;
            ...
            capacities.forEach(cap => {
                ...
                btn.innerHTML = `<span>${cap}</span><span class="text-xs font-normal text-gray-500">${v ? formatPrice(v.price) : ''}</span>`;
                btn.onclick = () => {
...
function renderProductGallery(images, activeImage) {
    ...
    if (!activeImage && images.length > 0) activeImage = images[0];
    if (activeImage) mainImg.src = activeImage;

    galleryContainer.innerHTML = images.map((imgUrl) => `
        <div class="w-16 h-16 border-2 ${imgUrl === activeImage ? 'border-blue-500' : 'border-transparent'} hover:border-blue-500 rounded-lg overflow-hidden cursor-pointer transition-all flex-shrink-0 bg-white" onclick="changeMainImage(this, '${imgUrl}')">
...
function findAndApplyVariant() {
    ...
    if (selectedColor) {
        if (colorGalleries[selectedColor] && colorGalleries[selectedColor].length > 0) {
            ...
            if (keyFound) images = colorGalleries[keyFound];
        }
    }

    if (images.length === 0 && variant && variant.image) images = [variant.image];
    if (images.length === 0) images = productState.baseGallery;
...
function updateBuyButtonState(isValid, stock) {
    ...
    if (stockStatus) {
        ...
        else if (stock > 0) stockStatus.innerHTML = `<span class="text-green-600 font-bold"><i class="fa-solid fa-check"></i> Còn hàng (${stock})</span>`;
        else stockStatus.innerHTML = `<span class="text-red-500 font-bold"><i class="fa-solid fa-xmark"></i> Hết hàng</span>`;
    }

    if (!btnAdd || !btnBuy) return;

    if (!isValid) {
        btnAdd.disabled = true; btnAdd.textContent = 'Chọn phiên bản'; btnAdd.className = "w-full bg-gray-200 text-gray-500 border-2 border-gray-300 p-3 rounded-lg font-bold cursor-not-allowed";
        btnBuy.disabled = true; btnBuy.className = "w-full bg-gray-300 text-gray-500 p-3 rounded-lg font-bold text-lg cursor-not-allowed shadow-none";
        return;
    }

    if (stock > 0) {
        btnAdd.disabled = false; btnAdd.textContent = 'Thêm vào giỏ'; btnAdd.className = "w-full bg-white text-blue-600 border-2 border-blue-600 p-3 rounded-lg font-bold hover:bg-blue-50 transition";
        btnBuy.disabled = false; btnBuy.className = "w-full bg-red-600 text-white p-3 rounded-lg font-bold text-lg hover:bg-red-700 transition shadow-md active:scale-95";
    } else {
        btnAdd.disabled = true; btnAdd.textContent = 'Hết hàng'; btnAdd.className = "w-full bg-gray-200 text-gray-500 border-2 border-gray-300 p-3 rounded-lg font-bold cursor-not-allowed";
        btnBuy.disabled = true; btnBuy.className = "w-full bg-gray-300 text-gray-500 p-3 rounded-lg font-bold text-lg cursor-not-allowed shadow-none";
    }
}
...
async function initProductDetailPage() {
    ...
    // 2. Gọi module Reviews (Chạy song song)
    initReviews(id); 
    ...
    try {
        ...
        // --- XỬ LÝ CHỌN TRƯỚC BIẾN THỂ TỪ URL ---
...
async function initCategoryPage() {
    ...
    if (titleEl) titleEl.textContent = currentCategorySlug === 'all' ? 'Tất cả sản phẩm' : `Danh mục: ${currentCategorySlug.toUpperCase()}`;
...
async function loadProductsWithFilter() {
    ...
    try {
        ...
        if (!products.length) { container.innerHTML = `<div class="col-span-full text-center py-10"><p class="text-gray-500 mb-2">Không tìm thấy sản phẩm phù hợp.</p><button onclick="location.reload()" class="text-blue-600 hover:underline text-sm">Xóa bộ lọc</button></div>`; return; }
        container.innerHTML = products.map(productCardHTML).join('');
        attachProductEvents(container);
        document.getElementById('filter-sidebar')?.classList.remove('active');
    } catch (err) { console.error(err); container.innerHTML = '<p class="col-span-full text-center text-red-500">Lỗi tải sản phẩm.</p>'; }
}
