    ...
    <div class="bg-white rounded-xl border border-gray-200 p-3 flex flex-col h-full relative group hover:shadow-lg transition-shadow duration-300">
      ...
      <div class="mt-3 flex-1 flex flex-col">
        ...
        <div class="mt-auto pt-2">
            <div class="flex flex-wrap items-baseline gap-2">
                <span class="text-lg font-bold ${isOutOfStock ? 'text-gray-500' : 'text-blue-600'}">${formatPrice(p.price)}</span>
      ...
      <div class="flex gap-2 mt-4">
        ...
        <button 
          ...
          title="Thêm vào giỏ" data-id="${p.id}" ${isOutOfStock ? 'disabled' : ''}>
        ...
        <button class="w-10 h-10 flex items-center justify-center bg-gray-100 text-gray-600 rounded-lg hover:bg-red-50 hover:text-red-500 active:scale-95 transition-colors border border-transparent hover:border-red-200 btn-add-to-wishlist" title="Thêm vào yêu thích" data-id="${p.id}">
...
async function handleAddToCartClick(btnElement, productId) {
  ...
  try {
    ...
    btnElement.classList.remove('bg-gray-100', 'text-gray-600', 'hover:bg-blue-50');
    ...
    setTimeout(() => {
        ...
        btnElement.classList.add('bg-gray-100', 'text-gray-600', 'hover:bg-blue-50');
...
async function handleBuyNowClick(productId) {
  ...
  try {
    ...
    localStorage.setItem('checkout_selected_items', JSON.stringify(itemsToCheckout));
...
async function initProductDetailPage() {
  ...
  try {
    ...
    if(product.compare_at) document.querySelector('#product-compare-price').textContent = formatPrice(product.compare_at);
    document.querySelector('#product-description').innerHTML = product.description?.replace(/\n/g, '<br>') || 'Chưa có mô tả.';
    ...
    if (stockEl) {
        if (stockQty > 0) {
            ...
            if (btnAdd) {
                ...
                newBtn.addEventListener('click', (e) => handleAddToCartClick(e.currentTarget, product.id));
            }
            if (btnBuy) {
                const newBtn = btnBuy.cloneNode(true);
                btnBuy.parentNode.replaceChild(newBtn, btnBuy);
                newBtn.addEventListener('click', () => handleBuyNowClick(product.id));
            }
        } else {
            ...
            if(btnAdd) {
                ...
                btnAdd.className = "w-full bg-gray-200 text-gray-500 border-2 border-gray-300 p-3 rounded-lg font-bold cursor-not-allowed flex items-center justify-center gap-2";
            }
            if(btnBuy) {
    ...
    if (breadcrumb) {
        breadcrumb.innerHTML = `
            ...
            <a href="category.html?slug=${product.category}" class="hover:text-blue-600 capitalize">${product.category || 'Sản phẩm'}</a>
            <span class="text-gray-300 mx-2">/</span>
    ...
    galleryContainer.innerHTML = images.map((imgUrl, index) => `
        <div class="w-16 h-16 border-2 border-transparent hover:border-blue-500 rounded-lg overflow-hidden cursor-pointer transition-all ${index === 0 ? 'border-blue-500' : ''}" 
    ...
    window.changeMainImage = (thumbEl, newSrc) => {
        mainImg.src = newSrc;
        document.querySelectorAll('#gallery-container > div').forEach(div => div.classList.remove('border-blue-500', 'border-transparent'));
...
function renderSpecs(specsJson, specsTemplateJson, category) {
    ...
    if (!template || template.length === 0) { template = SPECS_MAPPING[category] || SPECS_MAPPING['default']; }
    ...
    template.forEach((group) => {
        ...
        group.fields.forEach((field) => {
            ...
            let value = specs[key];
            if (!value) value = '<span class="text-gray-400 italic text-xs">Đang cập nhật</span>';
...
async function loadProductsWithFilter() {
    ...
    try {
        ...
        if (!products.length) {
            container.innerHTML = `<div class="col-span-full text-center py-10"><p class="text-gray-500 mb-2">Không tìm thấy sản phẩm phù hợp.</p><button onclick="location.reload()" class="text-blue-600 hover:underline text-sm">Xóa bộ lọc</button></div>`;
            return;
        }
