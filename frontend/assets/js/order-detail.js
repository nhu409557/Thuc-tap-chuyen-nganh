...
async function loadOrderDetail() {
  ...
  try {
    ...
    let subtotalCalculator = 0; // Biến tạm tính tổng tiền hàng (trước giảm giá)
      ...
      itemsContainer.innerHTML = items.map((item) => {
          ...
          if (item.variant_attributes) {
            try {
              ...
              const specs = Object.entries(attrs).filter(([k, v]) => k !== 'color' && v).map(([k, v]) => v);
              if (specs.length) variantParts.push(specs.join(' / '));
            } catch (e) {}
          }
          const variantText = variantParts.join(' - ');
          const variantHtml = variantText ? `<span class="text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded border border-gray-200 mt-1 inline-block">${variantText}</span>` : '';
    ...
    // --- TÍNH TOÁN & HIỂN THỊ TIỀN (BAO GỒM DISCOUNT) ---
    // Backend trả về order.total_amount là số tiền cuối cùng (Đã trừ discount)
    // Backend trả về order.discount_amount là số tiền đã giảm
    // => Tiền hàng (Subtotal) = Total Amount + Discount Amount
    
    const discountVal = parseInt(order.discount_amount || 0);
    const finalTotal = parseInt(order.total_amount);
    
    // Sử dụng subtotalCalculator từ vòng lặp (chính xác hơn) hoặc tính ngược lại
    ...
    if (discountVal > 0) {
        ...
        couponCodeEl.textContent = order.coupon_code || '';
    } else {
