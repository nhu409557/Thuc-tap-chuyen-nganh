...
class Product extends BaseModel
{
    ...
    public static function search(?string $categorySlug, ?string $q, int $page = 1, int $perPage = 16, $minPrice = null, $maxPrice = null, $brandName = null, $sort = 'newest', $status = 'active'): array
    {
        ...
        // 2. Lọc theo Danh mục (Filter từ Sidebar/Menu)
        if ($categorySlug && $categorySlug !== 'all') {
            $where[] = 'c.slug = ?';
            $params[] = $categorySlug;
        }

        // 3. TÌM KIẾM TỪ KHÓA (Logic mới: Tên, Hãng, Danh mục, Cấu hình, Mô tả)
        if ($q) {
            $keyword = '%' . trim($q) . '%';
            
            // Cập nhật logic tìm kiếm tại đây:
            // - p.title: Tên sản phẩm
            // - b.name: Tên hãng (VD: tìm 'Apple')
            // - c.name: Tên danh mục (VD: tìm 'Điện thoại')
            // - p.specs: Cấu hình (VD: tìm '256GB', 'Snapdragon')
            // - p.description: Mô tả chi tiết
            $where[] = '(p.title LIKE ? OR b.name LIKE ? OR c.name LIKE ? OR p.specs LIKE ? OR p.description LIKE ?)';
            
            $params[] = $keyword; // title
            $params[] = $keyword; // brand name
            $params[] = $keyword; // category name
            $params[] = $keyword; // specs (JSON string)
        ...
        // 5. Lọc thương hiệu (Sidebar Filter)
        if ($brandName) {
